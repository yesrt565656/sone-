...
Window:Toggle("Silent Aim", "Стреляет в ближайшего игрока к вашему курсору", false, function(State)
   Info.SilentAIMEnabled = State
end)

Window:Toggle("Trigger Bot(Автоматически стреляет)", "lag", false, function(State)
   Info.TriggeredEnabled = State
end)

Window:Divider("Settings")

Window:Slider("Fov", "Рекомендовано: 250", 10, 500, 250, function(Value)
   Info.FieldOfView = Value
end)

Window:Slider("Accuracy", "Просто точность стрельбы", 1, 100, 100, function(Value)
    Info.Accuracy = Value
end)

-- Обновляем список частей тела
Window:Dropdown("Aim Target", "Выберите часть тела, в которую Silent aim будет стрелять", 
...
Window:Dropdown("Team Whitelist", "Какую команду Silent aim будет игнорировать", {"Guards", "Inmates", "Criminals"}, function(Value)
   Info.TeamWhitelist = Value
end)

Window:Toggle("Danger Check", "Проверяет есть ли у заключенного оружие", false, function(State)
...
Window:Button("Kill All", "Убивает всех в игре", function()
...
Window:Divider("Модификация оружия")

-- Добавляем переключатель AutoFire
Window:Toggle("Auto Fire", "Автострельба (Не trigger bot)", false, function(State)
...
Window:Button("Get Guns", "Получить все оружия(В данный момент не работает)", function()
...
-- [ Credits ] --
Window:Divider("Создатель скрипта")

Window:Button("Tg: kryt224 ", "Создатель скрипта (нажмите чтобы скопировать)", function()
